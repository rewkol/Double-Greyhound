...
public class PuppetShadowController : MonoBehaviour, IPuppet
{
    ...
    void FixedUpdate()
    {
        ...
        if (state == 1 && timer == 0)
        {
            ...
            float chance = Random.Range(xBase - 9.0f, xBase + 1.0f);
            if (chance > distance + 2)
            {
                if (Random.value < (xBase - distance) / 12.0f)
                {
    ...
    private IEnumerator AttackRoutine()
    {
        ...
        sfxController.PlaySFX2D("General/Punch", 0.2f, 200, 0.15f, false);
        for (int i = 0; i < 24; i++)
        {
            yield return new WaitForFixedUpdate();
        }
    ...
    private IEnumerator HurtRoutine()
    {
        ...
        {
            sfxController.PlaySFX2D("General/Death", 0.1f, 200, 0.15f, false);
    ...
    private IEnumerator VictoryRoutine()
    {
        ...
        {
            sfxController.PlaySFX2D("SJHS/Puppet_Cheer", 0.2f, 210, 0.1f, false);
        }
